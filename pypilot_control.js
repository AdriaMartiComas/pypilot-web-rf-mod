...
//                    location.reload();
    ...
    //Adri code
    // Conectar al nuevo servidor Socket.IO en mando_rf.py
    var rf_socket = io.connect('http://localhost:5000');

    // Manejador para el evento del botón 1 del mando RF
    rf_socket.on('activate_autopilot', function() {
    ...
    // Manejador para el evento del botón 2 del mando RF
    rf_socket.on('deactivate_autopilot', function() {
        if($('.toggle-button').hasClass('toggle-button-selected')) {
            pypilot_set('ap.enabled', false);
        }
    });

    // Manejador para el evento del botón 3 del mando RF
    rf_socket.on('decrement_heading', function(data) {
        var value = data.value;
        var currentHeading = parseInt($('#heading_command').text());
        var newHeading = currentHeading - value;
        pypilot_set('ap.heading_command', newHeading);
    });

    // Manejador para el evento del botón 4 del mando RF
    rf_socket.on('increment_heading', function(data) {
        var value = data.value;
        var currentHeading = parseInt($('#heading_command').text());
        var newHeading = currentHeading + value;
        pypilot_set('ap.heading_command', newHeading);
    ...
    // Manejador para el evento del botón 5 del mando RF
    rf_socket.on('decrement_heading_large', function(data) {
        var value = data.value;
        var currentHeading = parseInt($('#heading_command').text());
        var newHeading = currentHeading - value;
        pypilot_set('ap.heading_command', newHeading);
    ...
    // Manejador para el evento del botón 6 del mando RF
    rf_socket.on('increment_heading_large', function(data) {
        var value = data.value;
        var currentHeading = parseInt($('#heading_command').text());
        var newHeading = currentHeading + value;
        pypilot_set('ap.heading_command', newHeading);
